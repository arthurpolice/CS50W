...
function makePageBar(posts) {
  ...
  const preexistingButtons = document.querySelectorAll('.page-number')
  ...
  const pageNavbar = document.querySelector('.pagination')
  ...
  const numberOfPages = posts['pages']
...
function navItemListenerHandler(li, posts) {
  ...
  if (posts['source'] === 'homepage' || posts['source'] === 'all_posts') {
    li.addEventListener('click', (ev) => {
      const page = ev.target.innerHTML
